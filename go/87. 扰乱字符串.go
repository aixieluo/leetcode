...
func main() {
	fmt.Println(isScramble("abb", "bba"))
...
func dfs(s1, s2 string) bool {
	...
	if hasCode(s1) != hasCode(s2) {
		mem[key] = false
		return false
	}

	for index := 1; index < len(s1); index++ {
		if dfs(s1[:index], s2[:index]) && dfs(s1[index:], s2[index:]) {
			mem[key] = true
			return true
		}
		if dfs(s1[:index], s2[len(s1)-index:]) && dfs(s1[index:], s2[:len(s1)-index]) {
			mem[key] = true
			return true
		}
	}
